...
CREATE (cd1)-[:CONECTA {tiempo_minutos: 10, trafico_actual: 'medio', capacidad: 30, activa: true}]->(z3);

MATCH (cd1:CentroDistribucion {nombre: 'Centro Principal'})
MATCH (z10:Zona {nombre: 'Paseo Caroni'})
WHERE NOT EXISTS((cd1)-[:CONECTA]->(z10))
CREATE (cd1)-[:CONECTA {tiempo_minutos: 15, trafico_actual: 'bajo', capacidad: 25, activa: true}]->(z10);
...
CREATE (cd2)-[:CONECTA {tiempo_minutos: 8, trafico_actual: 'alto', capacidad: 20, activa: true}]->(z6);

MATCH (cd2:CentroDistribucion {nombre: 'Centro Secundario'})
MATCH (z7:Zona {nombre: 'Unare'})
WHERE NOT EXISTS((cd2)-[:CONECTA]->(z7))
CREATE (cd2)-[:CONECTA {tiempo_minutos: 12, trafico_actual: 'medio', capacidad: 25, activa: true}]->(z7);


// Conexiones comerciales principales
MATCH (z1:Zona {nombre: 'AltaVista'}), (z7:Zona {nombre: 'Unare'})
WHERE NOT EXISTS((z1)-[:CONECTA]-(z7))
CREATE (z1)-[:CONECTA {tiempo_minutos: 18, trafico_actual: 'alto', capacidad: 35, activa: true}]->(z7)
CREATE (z7)-[:CONECTA {tiempo_minutos: 18, trafico_actual: 'alto', capacidad: 35, activa: true}]->(z1);

MATCH (z1:Zona {nombre: 'AltaVista'}), (z10:Zona {nombre: 'Paseo Caroni'})
WHERE NOT EXISTS((z1)-[:CONECTA]-(z10))
CREATE (z1)-[:CONECTA {tiempo_minutos: 12, trafico_actual: 'medio', capacidad: 25, activa: true}]->(z10)
CREATE (z10)-[:CONECTA {tiempo_minutos: 12, trafico_actual: 'medio', capacidad: 25, activa: true}]->(z1);

MATCH (z7:Zona {nombre: 'Unare'}), (z10:Zona {nombre: 'Paseo Caroni'})
WHERE NOT EXISTS((z7)-[:CONECTA]-(z10))
CREATE (z7)-[:CONECTA {tiempo_minutos: 15, trafico_actual: 'alto', capacidad: 30, activa: true}]->(z10)
CREATE (z10)-[:CONECTA {tiempo_minutos: 15, trafico_actual: 'alto', capacidad: 30, activa: true}]->(z7);
...
CREATE (z2)-[:CONECTA {tiempo_minutos: 7, trafico_actual: 'bajo', capacidad: 15, activa: true}]->(z4)
CREATE (z4)-[:CONECTA {tiempo_minutos: 7, trafico_actual: 'bajo', capacidad: 15, activa: true}]->(z2);

MATCH (z2:Zona {nombre: 'Castillito'}), (z5:Zona {nombre: 'Los Olivos'})
WHERE NOT EXISTS((z2)-[:CONECTA]-(z5))
CREATE (z2)-[:CONECTA {tiempo_minutos: 10, trafico_actual: 'bajo', capacidad: 20, activa: true}]->(z5)
CREATE (z5)-[:CONECTA {tiempo_minutos: 10, trafico_actual: 'bajo', capacidad: 20, activa: true}]->(z2);

MATCH (z4:Zona {nombre: 'Villa Asia'}), (z5:Zona {nombre: 'Los Olivos'})
WHERE NOT EXISTS((z4)-[:CONECTA]-(z5))
CREATE (z4)-[:CONECTA {tiempo_minutos: 5, trafico_actual: 'bajo', capacidad: 15, activa: true}]->(z5)
CREATE (z5)-[:CONECTA {tiempo_minutos: 5, trafico_actual: 'bajo', capacidad: 15, activa: true}]->(z4);


MATCH (z3:Zona {nombre: 'Puerto Ordaz'}), (z6:Zona {nombre: 'San Félix'})
WHERE NOT EXISTS((z3)-[:CONECTA]-(z6))
CREATE (z3)-[:CONECTA {tiempo_minutos: 20, trafico_actual: 'medio', capacidad: 25, activa: true}]->(z6);

MATCH (z3:Zona {nombre: 'Puerto Ordaz'}), (z2:Zona {nombre: 'Castillito'})
WHERE NOT EXISTS((z3)-[:CONECTA]-(z2))
CREATE (z3)-[:CONECTA {tiempo_minutos: 11, trafico_actual: 'bajo', capacidad: 6, activa: true}]->(z2)
CREATE (z2)-[:CONECTA {tiempo_minutos: 11, trafico_actual: 'bajo', capacidad: 6, activa: true}]->(z3);
...
ON CREATE SET r.tiempo_minutos = 40, r.trafico_actual = 'alto', r.capacidad = 30, r.activa = true;